...
`pip install -e .` registers the **`food`** command in your virtual environment so you can run `food list` instead of `python cli.py list`.
...
The **`food`** command sends HTTP requests to the running API. **Start the API first** (`python app.py`), then open a second terminal.

> `food` is installed via `pip install -e .`. You can still run `python cli.py` directly if you prefer.
...
| Command | Description |
|---------|-------------|
| `food list` | List all inventory items |
| `food show <id>` | Show details for one item |
| `food add "<name>"` | Add a new item manually |
| `food update <id>` | Update price, stock, or name |
| `food delete <id>` | Delete an item |
| `food find-barcode <barcode>` | Look up product on OpenFoodFacts |
| `food find-name "<name>"` | Search OpenFoodFacts by name |
| `food import` | Fetch from OpenFoodFacts and add to inventory |
...
# Add a manual item
food add "Green Tea" --brands Lipton --price 4.99 --stock 25

# Update stock and price
food update 1 --stock 50 --price 5.99

# Update product name only
food update 1 --name "Organic Green Tea"

# Delete item
food delete 2

# Search OpenFoodFacts (does not add to inventory)
food find-barcode 025293001301
food find-name "almond milk"

# Import from OpenFoodFacts into inventory
food import --barcode 025293001301 --price 4.99 --stock 10
food import --name "oat milk" --price 3.49 --stock 20
```
...
food --base-url http://127.0.0.1:5000 list

# Add with all optional fields
food add "Protein Bar" --brands "Clif" --barcode "1234567890" --ingredients "Oats, honey" --price 2.50 --stock 100
```
...
├── cli.py                    # CLI implementation (invoked via `food` command)
├── pyproject.toml            # Package config; registers `food` entry point
├── tests/
